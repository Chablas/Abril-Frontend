...
@Component({
  ...
  templateUrl: './area-item-edit.html',
})
export class AreaItemEdit implements OnInit {
  @Input() dto!: AreaItemEditDto;
  ...
  save(): void {
    ...
    this.loaderService.show();
    this.areaItemService.update(this.dto).subscribe({
      next: (res) => {
        this.loaderService.hide();
        Swal.fire({
          title: res.message ?? 'Área actualizada',
          icon: 'success',
