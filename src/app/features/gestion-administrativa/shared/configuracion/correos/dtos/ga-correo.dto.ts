 ...
 *    un rol (se expande a quien lo tenga hoy) o un correo escrito a mano. Se agregan, editan,
 *    prenden/apagan y eliminan desde la pantalla.
 */
...
/** Un rol para el desplegable del tipo ROL, con a cuánta gente alcanza hoy. */
export interface CorreoRolOption {
  roleId: number;
  nombre: string;
  /** A cuántos correos corporativos se expande hoy: en 0 no le llega a nadie. */
  miembros: number;
  /** Etiqueta del desplegable (nombre + cuánta gente); se calcula en el front. */
  label?: string;
}
