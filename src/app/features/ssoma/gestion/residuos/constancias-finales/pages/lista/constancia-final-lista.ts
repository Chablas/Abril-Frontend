...
import { ConstanciaFinalResiduoService } from '../../services/constancia-final-residuo.service';
...
@Component({
  selector: 'app-constancia-final-lista',
  standalone: true,
  ...
  templateUrl: './constancia-final-lista.html',
  styleUrl: './constancia-final-lista.css',
})
export class ConstanciaFinalLista implements OnInit {
  readonly tabs = RESIDUOS_TABS;
  items: ResiduoConstanciaFinalDto[] = [];
  ...
  private readonly pager = new ClientPager<ResiduoConstanciaFinalDto>();

  modalAbierto = false;
  guardando = false;
  editando: ResiduoConstanciaFinalDto | null = null;
  archivo: File | null = null;
  form: ResiduoConstanciaFinalUpsertDto = { projectId: 0, fechaEmision: '', observaciones: '' };

  constructor(
    private service: ConstanciaFinalResiduoService,
  ...
  load(): void {
    ...
    this.service.getAll(this.filtroProjectId ?? undefined).subscribe({
      next: (res) => {
        this.items = res;
        this.pager.reset();
        this.loading = false;
  ...
  get filtrosActivos(): number {
    return this.filtroProjectId !== null ? 1 : 0;
  }
  ...
  get pagedItems(): ResiduoConstanciaFinalDto[] {
  ...
  abrirNuevo(): void {
    this.editando = null;
    this.archivo = null;
    this.form = { projectId: 0, fechaEmision: '', observaciones: '' };
  ...
  guardar(): void {
    if (!this.form.projectId || !this.form.fechaEmision) return;
  ...
  eliminar(item: ResiduoConstanciaFinalDto): void {
    Swal.fire({
      icon: 'question',
      title: `¿Eliminar la constancia final #${item.id}?`,
