...
> Última actualización: 2026-06-03 — Módulo PASO frontend completo (ver Frontend CONTEXT). Backend pendiente: implementar controller unificado `SsomaPasoController` en `SsomaModule` con rutas `/api/v1/ssoma-paso`, `/api/v1/ssoma-paso/actividad`, `/api/v1/ssoma-paso/ejecucion`. Ver sección 7 para endpoints requeridos.
>
> **2026-05-29** — SctrVidaLeyRepository optimizado (LEFT JOIN habs + bulk vinculaciones), AprobarAsync fuerza Aprobado si sin pendientes, CatalogosRepository ListEmpresas sin filtro EsAbril
...
| `SsomaModule` | `AddSsomaModule` | EMO, programación, alertas automáticas, clínica, reportes SUNAFIL, **PASO (pendiente implementar)** |
...
## 7. Módulo PASO — Backend pendiente implementar

Controller: `SsomaPasoController` en `SsomaModule/PasoFeature/`. Ruta base: `api/v1/ssoma-paso`.

```
# Programas PASO
GET    /api/v1/ssoma-paso?anio=&proyectoId=&estado=&esPlantilla=   → PasoDto[]
GET    /api/v1/ssoma-paso/{id}                                     → PasoDto (include actividades + ejecuciones)
POST   /api/v1/ssoma-paso                                          body: CreatePasoDto
PUT    /api/v1/ssoma-paso/{id}                                     body: CreatePasoDto parcial
PATCH  /api/v1/ssoma-paso/{id}/aprobar                             → cambia estado a Aprobado
POST   /api/v1/ssoma-paso/{id}/instanciar                          body: InstanciarPasoDto { proyectoId, anio, nombre }
GET    /api/v1/ssoma-paso/{id}/gantt                               → PasoGanttDto { tasks[] }
GET    /api/v1/ssoma-paso/{id}/spi                                 → PasoSpiDto
GET    /api/v1/ssoma-paso/{id}/reporte?format=excel|pdf            → Blob
GET    /api/v1/ssoma-paso/dashboard                                → PasoDashboardDto
GET    /api/v1/ssoma-paso/alertas                                  → PasoAlertaDto[]
GET    /api/v1/ssoma-paso/categorias                               → PasoCategoriaDto[]

# Actividades
GET    /api/v1/ssoma-paso/actividad/{id}                           → PasoActividadDto (include ejecuciones)
POST   /api/v1/ssoma-paso/actividad                                body: CreateActividadDto
PUT    /api/v1/ssoma-paso/actividad/{id}                           body: CreateActividadDto parcial
DELETE /api/v1/ssoma-paso/actividad/{id}

# Ejecuciones
POST   /api/v1/ssoma-paso/ejecucion                                body: CreateEjecucionDto { actividadId, fechaEjecutada, observaciones?, participantesCount? }
PATCH  /api/v1/ssoma-paso/ejecucion/{id}/evidencia                 multipart/form-data, campo "file" → sube a Storage, guarda ruta
```

**DTOs de referencia** (frontend): `PasoDto`, `PasoActividadDto`, `PasoEjecucionDto`, `PasoSpiDto`, `PasoDashboardDto`, `PasoAlertaDto`, `PasoCategoriaDto`, `CreatePasoDto`, `InstanciarPasoDto`, `CreateActividadDto`, `CreateEjecucionDto` — todos en `features/ssoma/salud-ocupacional/paso/dtos/paso.dtos.ts`.

**Entidades sugeridas** (prefijo `SsPaso*`): `SsPaso`, `SsPasoActividad`, `SsPasoEjecucion`, `SsPasoCategoria`. Registrar DbSets en `AppDbContext`.

**Evaluaciones — Asignaciones**:
```
GET  /api/v1/evaluaciones/asignaciones-supervisor                → SupervisorAsignacionDto[]  (filtra subareas UDP + BIM)
GET  /api/v1/evaluaciones/asignaciones-supervisor/proyectos      → ProyectoAsignadoDto[] (todos los proyectos disponibles)
PUT  /api/v1/evaluaciones/asignaciones-supervisor/{workerId}     body: { projectIds: number[] }
```
