...
| `time-picker` | `app-time-picker` | Selector de hora con desplegable estilizado (columnas de hora/minuto); valor `HH:mm` 24h como el input nativo, `min` opcional (deshabilita las horas/minutos por debajo) y color de acento configurable (default `--color-abril-standard`) |
